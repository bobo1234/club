...
function analyzeBtns(v){
	...
	btns += secure.modify ? "<button type='button' class='btn btn-info btn-xs' onclick='initPassword("+v.id+")'><span class='glyphicon glyphicon-cog'></span>重置密码</button>" : "" ;
	btns += secure.modify && !v.initAccount ? "<button class='btn btn-success btn-xs' type='button' onclick='roleMgr(\""+v.name+"\")'><span class='glyphicon glyphicon glyphicon-star'></span>设置角色</button>" : "";
...
function addAccount(){
	...
	account.password = $.verifyFormForLength($('input.acctPass'), 5);
	if(!$.isSubmit) reutrn;
...
function showModifyBox(id){
	...
	$.getJSON(localhostUrl+'mgr/account/findAccountById', {id:id}, function(data){
		...
		$('select.stalist').val(data.body.acctState);
...
function mdoifyAccount(){
	...
	account.state = $.verifyForm($('select.stalist'), false);
	if(!$.isSubmit) return;
	dialog = BootstrapDialog.isSubmitted();
	$.post(localhostUrl+'mgr/account/modifyAccount', {
		id : account.id,
